...
export const OptionsBar = () => {
  ...
  return (
    <aside className=' h-screen bg-slate-200 border-l border-t border-slate-300 md:w-12 py-5 shadow-lg' >
        ...
        <>
          <img onClick={ () => setShowModalGestures(true) } src={ Gesture } alt='Gestos' className='mx-auto mb-6 h-6'/>
          { showModalGestures && createPortal( <ModalGestures/>, document.body) }
        </>
        <>
          <img onClick={ () => setShowModalSymbols(true) } src={ Symbols } alt='Símbolos' className='mx-auto mb-6 h-6'/>
          { showModalSymbols && createPortal( <ModalSymbols/>, document.body) }
        </>
        <>
          <img onClick={ () => setShowModalProfile(true) } src={ Profile } alt='Perfil' className='mx-auto mb-6 h-6'/>
