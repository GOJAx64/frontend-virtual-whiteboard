...
export const ModalAddParticipants = () => {
    const { name, description, summary, isFormValid, onInputChange, onResetForm} = useForm(formData, formValidations);
    const { alert, showAlert, submitClassroom, classroom, setShowModalAddParticipants } = useClassrooms();
            ...
            <div className="justify-center items-center flex overflow-x-hidden overflow-y-auto fixed inset-0 z-50 outline-none focus:outline-none">
                ...
                <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-slate-100 outline-none focus:outline-none">
                    ...
                    <h3 className="text-2xl font-medium text-slate-400">
                        Participantes en { classroom.name }
                    </h3>
                    <button className="p-1 ml-auto border-0 text-black  float-right text-3xl leading-none font-semibold outline-none focus:outline-none" onClick={ () => setShowModalAddParticipants(false) }>
                    ...
                    <button
                        ...
                        type="button"
                        onClick={() => setShowModalAddParticipants(false)}
                    >
                        Cerrar
                    </button>
                    <button
